...
//
// These functions are the whole public surface of libonedrive.a: everything a
...
// onedrive_openfile and onedrive_writefile are left to the driver's own tests.
//
...
func TestFreeToleratesNil(t *testing.T) {
	onedrive_free(nil) // must not crash
}
...
func TestMountRejectsInvalidJSON(t *testing.T) {
	...
	defer onedrive_free(bad)

	if got := onedrive_mount(1, bad); int(got) != -1 {
...
func TestMountRejectsEmptyConfig(t *testing.T) {
	empty := stringToC("{}")
	defer onedrive_free(empty)

	if got := onedrive_mount(1, empty); int(got) == 0 {
		_ = onedrive_unmount(1)
...
func TestOperationsOnUnmountedDriver(t *testing.T) {
	...
	defer onedrive_free(path)
	other := stringToC("/other/path")
	defer onedrive_free(other)

	t.Run("stat", func(t *testing.T) {
		out := stringToC("")
		onedrive_free(out)
		if rc := onedrive_stat(99, path, &out); int(rc) == 0 {
	...
	t.Run("listdir", func(t *testing.T) {
		out := stringToC("")
		onedrive_free(out)
		if rc := onedrive_listdir(99, path, &out); int(rc) == 0 {
			t.Error("listdir succeeded while unmounted")
		}
	})

	t.Run("mkdir", func(t *testing.T) {
		if rc := onedrive_mkdir(99, path); int(rc) == 0 {
			t.Error("mkdir succeeded while unmounted")
		}
	})

	t.Run("remove", func(t *testing.T) {
		if rc := onedrive_remove(99, path); int(rc) == 0 {
			t.Error("remove succeeded while unmounted")
		}
	})

	t.Run("rename", func(t *testing.T) {
		if rc := onedrive_rename(99, path, other); int(rc) == 0 {
	...
	t.Run("unmount is idempotent", func(t *testing.T) {
		if rc := onedrive_unmount(99); int(rc) != 0 {
