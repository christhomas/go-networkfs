...
//
// These functions are the whole public surface of libs3.a: everything a
...
// s3_openfile and s3_writefile are left to the driver's own tests.
//
...
func TestMountRejectsInvalidJSON(t *testing.T) {
	...
	if got := s3_mount(1, bad); int(got) != -1 {
...
func TestMountRejectsEmptyConfig(t *testing.T) {
	...
	if got := s3_mount(1, empty); int(got) == 0 {
		_ = s3_unmount(1)
...
func TestOperationsOnUnmountedDriver(t *testing.T) {
	...
	t.Run("stat", func(t *testing.T) {
		out := stringToC("")
		s3_free(out)
		if rc := s3_stat(99, path, &out); int(rc) == 0 {
	...
	t.Run("listdir", func(t *testing.T) {
		out := stringToC("")
		s3_free(out)
		if rc := s3_listdir(99, path, &out); int(rc) == 0 {
			t.Error("listdir succeeded while unmounted")
		}
	})

	t.Run("mkdir", func(t *testing.T) {
		if rc := s3_mkdir(99, path); int(rc) == 0 {
			t.Error("mkdir succeeded while unmounted")
		}
	})

	t.Run("remove", func(t *testing.T) {
		if rc := s3_remove(99, path); int(rc) == 0 {
			t.Error("remove succeeded while unmounted")
		}
	})

	t.Run("rename", func(t *testing.T) {
		if rc := s3_rename(99, path, other); int(rc) == 0 {
