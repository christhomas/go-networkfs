// A consumer's-eye test of libonedrive.a.
//
...
// onedrive_openfile, onedrive_writefile and setOutBytes take a ByteSlice or a
...
static void test_mount_rejects_bad_input(void) {
    check(onedrive_mount(1, "{not json") == -1, "malformed JSON returns -1");
    check(onedrive_mount(1, "{}") != 0, "an empty config is refused");
}
...
static void test_writefile_unmounted(void) {
    ...
    check(onedrive_writefile(99, "/nope", data) != 0, "writefile while unmounted fails");

    ByteSlice empty;
    empty.data = NULL;
    empty.len = 0;
    check(onedrive_writefile(99, "/nope", empty) != 0,
...
static void test_operations_unmounted(void) {
    ...
    check(onedrive_stat(99, "/p", &out) != 0, "stat while unmounted fails");
    if (out) { onedrive_free(out); out = NULL; }

    check(onedrive_listdir(99, "/p", &out) != 0, "listdir while unmounted fails");
    if (out) { onedrive_free(out); out = NULL; }

    check(onedrive_mkdir(99, "/p") != 0, "mkdir while unmounted fails");
    check(onedrive_remove(99, "/p") != 0, "remove while unmounted fails");
    check(onedrive_rename(99, "/a", "/b") != 0, "rename while unmounted fails");

    // Unmount is idempotent by design: tearing down what was never set up is
    // not an error.
    check(onedrive_unmount(99) == 0, "unmount of a never-mounted driver succeeds");
}
...
static void test_mounted_round_trip(void) {
    ...
    if (!cfg || cfg[0] == '\0') {
        printf("onedrive: no CABI_CONFIG; skipping the mounted tests\n");
        return;
    }

    const int id = 1;
    if (onedrive_mount(id, (char *)cfg) != 0) {
        check(0, "mount against the test server");
        return;
    }
    check(1, "mount against the test server");

    const char *path = "/cabi-onedrive-roundtrip.txt";
    const char *payload = "written through the C ABI";

    ByteSlice data;
    data.data = (char *)payload;
    data.len = strlen(payload);
    check(onedrive_writefile(id, (char *)path, data) == 0, "writefile succeeds");

    ByteSlice out;
    out.data = NULL;
    out.len = 0;
    check(onedrive_openfile(id, (char *)path, &out) == 0, "openfile succeeds");
    ...
    char *info = NULL;
    check(onedrive_stat(id, (char *)path, &info) == 0, "stat succeeds");
    if (info) {
        check(strstr(info, "cabi-onedrive-roundtrip.txt") != NULL, "stat names the file");
        onedrive_free(info);
    }

    char *listing = NULL;
    check(onedrive_listdir(id, "/", &listing) == 0, "listdir succeeds");
    if (listing) onedrive_free(listing);

    check(onedrive_remove(id, (char *)path) == 0, "remove succeeds");
    check(onedrive_unmount(id) == 0, "unmount succeeds");
}
...
extern int onedrive_flush_coverage(char *dir);
#endif
...
int main(void) {
    ...
    printf("onedrive: %d checks, %d failures\n", checks, failures);

#ifdef NETWORKFS_COVERAGE
    {
        const char *dir = getenv("GOCOVERDIR");
        if (dir && onedrive_flush_coverage((char *)dir) != 0) {
