// A consumer's-eye test of libs3.a.
//
...
// s3_openfile, s3_writefile and setOutBytes take a ByteSlice or a
...
static void test_mount_rejects_bad_input(void) {
    check(s3_mount(1, "{not json") == -1, "malformed JSON returns -1");
    check(s3_mount(1, "{}") != 0, "an empty config is refused");
}
...
static void test_writefile_unmounted(void) {
    ...
    check(s3_writefile(99, "/nope", data) != 0, "writefile while unmounted fails");

    ByteSlice empty;
    empty.data = NULL;
    empty.len = 0;
    check(s3_writefile(99, "/nope", empty) != 0,
...
static void test_operations_unmounted(void) {
    ...
    check(s3_stat(99, "/p", &out) != 0, "stat while unmounted fails");
    if (out) { s3_free(out); out = NULL; }

    check(s3_listdir(99, "/p", &out) != 0, "listdir while unmounted fails");
    if (out) { s3_free(out); out = NULL; }

    check(s3_mkdir(99, "/p") != 0, "mkdir while unmounted fails");
    check(s3_remove(99, "/p") != 0, "remove while unmounted fails");
    check(s3_rename(99, "/a", "/b") != 0, "rename while unmounted fails");

    // Unmount is idempotent by design: tearing down what was never set up is
    // not an error.
    check(s3_unmount(99) == 0, "unmount of a never-mounted driver succeeds");
}
...
static void test_mounted_round_trip(void) {
    ...
    if (!cfg || cfg[0] == '\0') {
        printf("s3: no CABI_CONFIG; skipping the mounted tests\n");
        return;
    }

    const int id = 1;
    if (s3_mount(id, (char *)cfg) != 0) {
        check(0, "mount against the test server");
        return;
    }
    check(1, "mount against the test server");

    const char *path = "/cabi-s3-roundtrip.txt";
    const char *payload = "written through the C ABI";

    ByteSlice data;
    data.data = (char *)payload;
    data.len = strlen(payload);
    check(s3_writefile(id, (char *)path, data) == 0, "writefile succeeds");

    ByteSlice out;
    out.data = NULL;
    out.len = 0;
    check(s3_openfile(id, (char *)path, &out) == 0, "openfile succeeds");
    ...
    char *info = NULL;
    check(s3_stat(id, (char *)path, &info) == 0, "stat succeeds");
    if (info) {
        check(strstr(info, "cabi-s3-roundtrip.txt") != NULL, "stat names the file");
        s3_free(info);
    }

    char *listing = NULL;
    check(s3_listdir(id, "/", &listing) == 0, "listdir succeeds");
    if (listing) s3_free(listing);

    check(s3_remove(id, (char *)path) == 0, "remove succeeds");
    check(s3_unmount(id) == 0, "unmount succeeds");
}
...
extern int s3_flush_coverage(char *dir);
#endif
...
int main(void) {
    ...
    printf("s3: %d checks, %d failures\n", checks, failures);

#ifdef NETWORKFS_COVERAGE
    {
        const char *dir = getenv("GOCOVERDIR");
        if (dir && s3_flush_coverage((char *)dir) != 0) {
