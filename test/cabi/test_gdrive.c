// A consumer's-eye test of libgdrive.a.
//
...
// gdrive_openfile, gdrive_writefile and setOutBytes take a ByteSlice or a
...
static void test_mount_rejects_bad_input(void) {
    check(gdrive_mount(1, "{not json") == -1, "malformed JSON returns -1");
    check(gdrive_mount(1, "{}") != 0, "an empty config is refused");
}
...
static void test_writefile_unmounted(void) {
    ...
    check(gdrive_writefile(99, "/nope", data) != 0, "writefile while unmounted fails");

    ByteSlice empty;
    empty.data = NULL;
    empty.len = 0;
    check(gdrive_writefile(99, "/nope", empty) != 0,
...
static void test_operations_unmounted(void) {
    ...
    check(gdrive_stat(99, "/p", &out) != 0, "stat while unmounted fails");
    if (out) { gdrive_free(out); out = NULL; }

    check(gdrive_listdir(99, "/p", &out) != 0, "listdir while unmounted fails");
    if (out) { gdrive_free(out); out = NULL; }

    check(gdrive_mkdir(99, "/p") != 0, "mkdir while unmounted fails");
    check(gdrive_remove(99, "/p") != 0, "remove while unmounted fails");
    check(gdrive_rename(99, "/a", "/b") != 0, "rename while unmounted fails");

    // Unmount is idempotent by design: tearing down what was never set up is
    // not an error.
    check(gdrive_unmount(99) == 0, "unmount of a never-mounted driver succeeds");
}
...
static void test_mounted_round_trip(void) {
    ...
    if (!cfg || cfg[0] == '\0') {
        printf("gdrive: no CABI_CONFIG; skipping the mounted tests\n");
        return;
    }

    const int id = 1;
    if (gdrive_mount(id, (char *)cfg) != 0) {
        check(0, "mount against the test server");
        return;
    }
    check(1, "mount against the test server");

    const char *path = "/cabi-gdrive-roundtrip.txt";
    const char *payload = "written through the C ABI";

    ByteSlice data;
    data.data = (char *)payload;
    data.len = strlen(payload);
    check(gdrive_writefile(id, (char *)path, data) == 0, "writefile succeeds");

    ByteSlice out;
    out.data = NULL;
    out.len = 0;
    check(gdrive_openfile(id, (char *)path, &out) == 0, "openfile succeeds");
    ...
    char *info = NULL;
    check(gdrive_stat(id, (char *)path, &info) == 0, "stat succeeds");
    if (info) {
        check(strstr(info, "cabi-gdrive-roundtrip.txt") != NULL, "stat names the file");
        gdrive_free(info);
    }

    char *listing = NULL;
    check(gdrive_listdir(id, "/", &listing) == 0, "listdir succeeds");
    if (listing) gdrive_free(listing);

    check(gdrive_remove(id, (char *)path) == 0, "remove succeeds");
    check(gdrive_unmount(id) == 0, "unmount succeeds");
}
...
extern int gdrive_flush_coverage(char *dir);
#endif
...
int main(void) {
    ...
    printf("gdrive: %d checks, %d failures\n", checks, failures);

#ifdef NETWORKFS_COVERAGE
    {
        const char *dir = getenv("GOCOVERDIR");
        if (dir && gdrive_flush_coverage((char *)dir) != 0) {
