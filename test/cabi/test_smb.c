// A consumer's-eye test of libsmb.a.
//
...
// smb_openfile, smb_writefile and setOutBytes take a ByteSlice or a
...
static void test_mount_rejects_bad_input(void) {
    check(smb_mount(1, "{not json") == -1, "malformed JSON returns -1");
    check(smb_mount(1, "{}") != 0, "an empty config is refused");
}
...
static void test_writefile_unmounted(void) {
    ...
    check(smb_writefile(99, "/nope", data) != 0, "writefile while unmounted fails");

    ByteSlice empty;
    empty.data = NULL;
    empty.len = 0;
    check(smb_writefile(99, "/nope", empty) != 0,
...
static void test_operations_unmounted(void) {
    ...
    check(smb_stat(99, "/p", &out) != 0, "stat while unmounted fails");
    if (out) { smb_free(out); out = NULL; }

    check(smb_listdir(99, "/p", &out) != 0, "listdir while unmounted fails");
    if (out) { smb_free(out); out = NULL; }

    check(smb_mkdir(99, "/p") != 0, "mkdir while unmounted fails");
    check(smb_remove(99, "/p") != 0, "remove while unmounted fails");
    check(smb_rename(99, "/a", "/b") != 0, "rename while unmounted fails");

    // Unmount is idempotent by design: tearing down what was never set up is
    // not an error.
    check(smb_unmount(99) == 0, "unmount of a never-mounted driver succeeds");
}
...
static void test_mounted_round_trip(void) {
    ...
    if (!cfg || cfg[0] == '\0') {
        printf("smb: no CABI_CONFIG; skipping the mounted tests\n");
        return;
    }

    const int id = 1;
    if (smb_mount(id, (char *)cfg) != 0) {
        check(0, "mount against the test server");
        return;
    }
    check(1, "mount against the test server");

    const char *path = "/cabi-smb-roundtrip.txt";
    const char *payload = "written through the C ABI";

    ByteSlice data;
    data.data = (char *)payload;
    data.len = strlen(payload);
    check(smb_writefile(id, (char *)path, data) == 0, "writefile succeeds");

    ByteSlice out;
    out.data = NULL;
    out.len = 0;
    check(smb_openfile(id, (char *)path, &out) == 0, "openfile succeeds");
    ...
    char *info = NULL;
    check(smb_stat(id, (char *)path, &info) == 0, "stat succeeds");
    if (info) {
        check(strstr(info, "cabi-smb-roundtrip.txt") != NULL, "stat names the file");
        smb_free(info);
    }

    char *listing = NULL;
    check(smb_listdir(id, "/", &listing) == 0, "listdir succeeds");
    if (listing) smb_free(listing);

    check(smb_remove(id, (char *)path) == 0, "remove succeeds");
    check(smb_unmount(id) == 0, "unmount succeeds");
}
...
extern int smb_flush_coverage(char *dir);
#endif
...
int main(void) {
    ...
    printf("smb: %d checks, %d failures\n", checks, failures);

#ifdef NETWORKFS_COVERAGE
    {
        const char *dir = getenv("GOCOVERDIR");
        if (dir && smb_flush_coverage((char *)dir) != 0) {
