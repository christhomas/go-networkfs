...
# See the fs-linux-test-harness README, "Output: quiet by default, --verbose on
# request".
#
...
# either way, and a tier that has outgrown its budget should say so whether or
...
[ -x "$BUDGET" ] || { echo "tier.sh: $BUDGET is missing or not executable" >&2; exit 1; }
...
# FLTH_VERBOSE itself, so mapping the flag onto it is all that is needed — and
# it means the environment variable and the flag cannot disagree.
case " ${CLI_ARGS:-} " in
