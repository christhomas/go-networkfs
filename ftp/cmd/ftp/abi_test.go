...
//
// These functions are the whole public surface of libftp.a: everything a
...
// ftp_openfile and ftp_writefile are left to the driver's own tests.
//
...
func TestMountRejectsInvalidJSON(t *testing.T) {
	...
	if got := ftp_mount(1, bad); int(got) != -1 {
...
func TestMountRejectsEmptyConfig(t *testing.T) {
	empty := stringToC("{}")
	defer ftp_free(empty)

	if got := ftp_mount(1, empty); int(got) == 0 {
		_ = ftp_unmount(1)
...
func TestOperationsOnUnmountedDriver(t *testing.T) {
	...
	defer ftp_free(other)

	t.Run("stat", func(t *testing.T) {
		out := stringToC("")
		ftp_free(out)
		if rc := ftp_stat(99, path, &out); int(rc) == 0 {
	...
	t.Run("listdir", func(t *testing.T) {
		out := stringToC("")
		ftp_free(out)
		if rc := ftp_listdir(99, path, &out); int(rc) == 0 {
			t.Error("listdir succeeded while unmounted")
		}
	})

	t.Run("mkdir", func(t *testing.T) {
		if rc := ftp_mkdir(99, path); int(rc) == 0 {
			t.Error("mkdir succeeded while unmounted")
		}
	})

	t.Run("remove", func(t *testing.T) {
		if rc := ftp_remove(99, path); int(rc) == 0 {
			t.Error("remove succeeded while unmounted")
		}
	})

	t.Run("rename", func(t *testing.T) {
		if rc := ftp_rename(99, path, other); int(rc) == 0 {
