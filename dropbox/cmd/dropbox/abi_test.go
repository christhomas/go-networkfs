...
//
// These functions are the whole public surface of libdropbox.a: everything a
...
// dropbox_openfile and dropbox_writefile are left to the driver's own tests.
//
...
func TestFreeToleratesNil(t *testing.T) {
	dropbox_free(nil) // must not crash
}
...
func TestMountRejectsInvalidJSON(t *testing.T) {
	...
	defer dropbox_free(bad)

	if got := dropbox_mount(1, bad); int(got) != -1 {
...
func TestMountRejectsEmptyConfig(t *testing.T) {
	empty := stringToC("{}")
	defer dropbox_free(empty)

	if got := dropbox_mount(1, empty); int(got) == 0 {
		_ = dropbox_unmount(1)
...
func TestOperationsOnUnmountedDriver(t *testing.T) {
	...
	defer dropbox_free(path)
	other := stringToC("/other/path")
	defer dropbox_free(other)

	t.Run("stat", func(t *testing.T) {
		out := stringToC("")
		dropbox_free(out)
		if rc := dropbox_stat(99, path, &out); int(rc) == 0 {
	...
	t.Run("listdir", func(t *testing.T) {
		out := stringToC("")
		dropbox_free(out)
		if rc := dropbox_listdir(99, path, &out); int(rc) == 0 {
			t.Error("listdir succeeded while unmounted")
		}
	})

	t.Run("mkdir", func(t *testing.T) {
		if rc := dropbox_mkdir(99, path); int(rc) == 0 {
			t.Error("mkdir succeeded while unmounted")
		}
	})

	t.Run("remove", func(t *testing.T) {
		if rc := dropbox_remove(99, path); int(rc) == 0 {
			t.Error("remove succeeded while unmounted")
		}
	})

	t.Run("rename", func(t *testing.T) {
		if rc := dropbox_rename(99, path, other); int(rc) == 0 {
	...
	t.Run("unmount is idempotent", func(t *testing.T) {
		if rc := dropbox_unmount(99); int(rc) != 0 {
