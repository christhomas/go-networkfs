...
//
// These functions are the whole public surface of libsmb.a: everything a
...
// smb_openfile and smb_writefile are left to the driver's own tests.
//
...
func TestMountRejectsInvalidJSON(t *testing.T) {
	...
	if got := smb_mount(1, bad); int(got) != -1 {
...
func TestMountRejectsEmptyConfig(t *testing.T) {
	empty := stringToC("{}")
	defer smb_free(empty)

	if got := smb_mount(1, empty); int(got) == 0 {
		_ = smb_unmount(1)
...
func TestOperationsOnUnmountedDriver(t *testing.T) {
	...
	defer smb_free(other)

	t.Run("stat", func(t *testing.T) {
		out := stringToC("")
		smb_free(out)
		if rc := smb_stat(99, path, &out); int(rc) == 0 {
	...
	t.Run("listdir", func(t *testing.T) {
		out := stringToC("")
		smb_free(out)
		if rc := smb_listdir(99, path, &out); int(rc) == 0 {
			t.Error("listdir succeeded while unmounted")
		}
	})

	t.Run("mkdir", func(t *testing.T) {
		if rc := smb_mkdir(99, path); int(rc) == 0 {
			t.Error("mkdir succeeded while unmounted")
		}
	})

	t.Run("remove", func(t *testing.T) {
		if rc := smb_remove(99, path); int(rc) == 0 {
			t.Error("remove succeeded while unmounted")
		}
	})

	t.Run("rename", func(t *testing.T) {
		if rc := smb_rename(99, path, other); int(rc) == 0 {
