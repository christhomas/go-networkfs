// smb/cmd/smb/main.go - SMB driver C library exports
//
// Builds as: go build -buildmode=c-archive -o libsmb.a ./smb/cmd/smb
// Exports: smb_version, smb_mount, smb_unmount, smb_stat, smb_listdir,
// smb_openfile, smb_writefile, smb_mkdir, smb_remove, smb_rename,
// smb_free.
//
...
import (
	...
	"github.com/christhomas/go-networkfs/smb"
)

var driver = &smb.SMBDriver{}
...
//export smb_version
func smb_version() *C.char {
	return stringToC("1.0.0")
}

// mountID: unique identifier for this mount instance
// configJSON: {"host":"...","port":"445","user":"...","pass":"...","share":"...","root":"/"}
// Returns: 0 on success, error code on failure
//
//export smb_mount
func smb_mount(mountID C.int, configJSON *C.char) C.int {
...
//export smb_unmount
func smb_unmount(mountID C.int) C.int {
...
// Returns JSON FileInfo or error (caller must free with smb_free)
//
//export smb_stat
func smb_stat(mountID C.int, path *C.char, outJSON **C.char) C.int {
...
// Returns JSON array of FileInfo (caller must free with smb_free)
//
//export smb_listdir
func smb_listdir(mountID C.int, path *C.char, outJSON **C.char) C.int {
...
// Returns file data in out buffer (caller must free with smb_free)
//
//export smb_openfile
func smb_openfile(mountID C.int, path *C.char, out *C.ByteSlice) C.int {
...
//
//export smb_writefile
func smb_writefile(mountID C.int, path *C.char, data C.ByteSlice) C.int {
...
//export smb_mkdir
func smb_mkdir(mountID C.int, path *C.char) C.int {
...
//export smb_remove
func smb_remove(mountID C.int, path *C.char) C.int {
...
//export smb_rename
func smb_rename(mountID C.int, oldPath *C.char, newPath *C.char) C.int {
...
// Frees memory allocated by smb_stat, smb_listdir, smb_openfile,
// smb_writefile, smb_version. Safe to call with NULL.
//
//export smb_free
func smb_free(ptr *C.char) {
	if ptr != nil {
