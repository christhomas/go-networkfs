...
//
// These functions are the whole public surface of libgdrive.a: everything a
...
// gdrive_openfile and gdrive_writefile are left to the driver's own tests.
//
...
func TestFreeToleratesNil(t *testing.T) {
	gdrive_free(nil) // must not crash
}
...
func TestMountRejectsInvalidJSON(t *testing.T) {
	...
	defer gdrive_free(bad)

	if got := gdrive_mount(1, bad); int(got) != -1 {
...
func TestMountRejectsEmptyConfig(t *testing.T) {
	empty := stringToC("{}")
	defer gdrive_free(empty)

	if got := gdrive_mount(1, empty); int(got) == 0 {
		_ = gdrive_unmount(1)
...
func TestOperationsOnUnmountedDriver(t *testing.T) {
	...
	defer gdrive_free(path)
	other := stringToC("/other/path")
	defer gdrive_free(other)

	t.Run("stat", func(t *testing.T) {
		out := stringToC("")
		gdrive_free(out)
		if rc := gdrive_stat(99, path, &out); int(rc) == 0 {
	...
	t.Run("listdir", func(t *testing.T) {
		out := stringToC("")
		gdrive_free(out)
		if rc := gdrive_listdir(99, path, &out); int(rc) == 0 {
			t.Error("listdir succeeded while unmounted")
		}
	})

	t.Run("mkdir", func(t *testing.T) {
		if rc := gdrive_mkdir(99, path); int(rc) == 0 {
			t.Error("mkdir succeeded while unmounted")
		}
	})

	t.Run("remove", func(t *testing.T) {
		if rc := gdrive_remove(99, path); int(rc) == 0 {
			t.Error("remove succeeded while unmounted")
		}
	})

	t.Run("rename", func(t *testing.T) {
		if rc := gdrive_rename(99, path, other); int(rc) == 0 {
	...
	t.Run("unmount is idempotent", func(t *testing.T) {
		if rc := gdrive_unmount(99); int(rc) != 0 {
