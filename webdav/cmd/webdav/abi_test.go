...
//
// These functions are the whole public surface of libwebdav.a: everything a
...
// webdav_openfile and webdav_writefile are left to the driver's own tests.
//
...
func TestFreeToleratesNil(t *testing.T) {
	webdav_free(nil) // must not crash
}
...
func TestMountRejectsInvalidJSON(t *testing.T) {
	...
	defer webdav_free(bad)

	if got := webdav_mount(1, bad); int(got) != -1 {
...
func TestMountRejectsEmptyConfig(t *testing.T) {
	empty := stringToC("{}")
	defer webdav_free(empty)

	if got := webdav_mount(1, empty); int(got) == 0 {
		_ = webdav_unmount(1)
...
func TestOperationsOnUnmountedDriver(t *testing.T) {
	...
	defer webdav_free(path)
	other := stringToC("/other/path")
	defer webdav_free(other)

	t.Run("stat", func(t *testing.T) {
		out := stringToC("")
		webdav_free(out)
		if rc := webdav_stat(99, path, &out); int(rc) == 0 {
	...
	t.Run("listdir", func(t *testing.T) {
		out := stringToC("")
		webdav_free(out)
		if rc := webdav_listdir(99, path, &out); int(rc) == 0 {
			t.Error("listdir succeeded while unmounted")
		}
	})

	t.Run("mkdir", func(t *testing.T) {
		if rc := webdav_mkdir(99, path); int(rc) == 0 {
			t.Error("mkdir succeeded while unmounted")
		}
	})

	t.Run("remove", func(t *testing.T) {
		if rc := webdav_remove(99, path); int(rc) == 0 {
			t.Error("remove succeeded while unmounted")
		}
	})

	t.Run("rename", func(t *testing.T) {
		if rc := webdav_rename(99, path, other); int(rc) == 0 {
	...
	t.Run("unmount is idempotent", func(t *testing.T) {
		if rc := webdav_unmount(99); int(rc) != 0 {
